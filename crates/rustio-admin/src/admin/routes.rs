...
const ADMIN_CSS: &str = concat!(
    ...
    "\n",
    include_str!("../../assets/static/admin/components/page-header.css"),
    "\n",
...
/// Detect a `RUSTIO_TOKENS_CSS` override that breaks the light-only contract.
///
/// RustIO Admin ships **one theme**: light. The baked bundle has no
/// `[data-theme="dark"]` block and no `prefers-color-scheme` block.
///
/// An override is appended *after* that bundle, so any dark block it
/// carries still applies in the browser — a `@media (prefers-color-scheme:
/// dark)` rule matches on the operator's OS setting regardless of what the
/// framework ships. An override with dark rules therefore repaints part of
/// a light-only admin dark, which is the mismatch this detects: dark
/// surfaces under light chrome the framework never re-derives.
///
/// Detection only. The override is never rewritten or wrapped — the
/// operator's CSS is theirs (Doctrine).
fn override_declares_dark_rules(css: &str) -> bool {
    ...
    defines_root_colors && declares_dark
}
...
/// be read. An override carrying dark rules logs a WARN (see
/// [`override_declares_dark_rules`]) but is still served verbatim.
fn load_token_override() -> Option<String> {
    let path = std::env::var("RUSTIO_TOKENS_CSS").ok()?;
    match std::fs::read_to_string(&path) {
        Ok(css) => {
            if override_declares_dark_rules(&css) {
                log::warn!(
                    "RUSTIO_TOKENS_CSS={path:?} declares dark rules ([data-theme=\"dark\"] \
                     or prefers-color-scheme), but RustIO Admin is light-only. The override \
                     is appended after the baked bundle, so those rules still apply in the \
                     browser and will repaint part of the admin dark while the framework \
                     chrome stays light. Regenerate the palette without dark blocks."
                );
            }
            Some(css)
        }
...
mod tests {
    ...
    // Light-only contract guard. RustIO Admin ships one theme; these
    // fixtures are the two real override shapes a generator emits.

    /// A light-only generated `tokens.css`: `:root` brand + surface
    /// tokens, no dark handling. This is what a correct override looks
    /// like now — it must NOT be flagged.
    const GENERATED_LIGHT_ONLY: &str = "/* GENERATED by a token generator */\n\
        :root {\n  --rio-brand-light: #0e6b5b;\n  --rio-accent: #0e6b5b;\n  \
        --rio-bg: #f7f5f2;\n  --rio-surface: #ffffff;\n}\n";

    /// A dual-block emission: `:root` light + an explicit
    /// `[data-theme="dark"]` block + a `prefers-color-scheme` auto block.
    /// Against a light-only framework this is the hazard.
    const DECLARES_DARK: &str = "/* Generated by a dark-aware generator */\n\
        :root {\n  --rio-accent: #0e6b5b;\n  --rio-bg: #f7f5f2;\n}\n\
        :root[data-theme=\"dark\"] {\n  --rio-bg: #0f172a;\n}\n\
        @media (prefers-color-scheme: dark) {\n  :root {\n    --rio-bg: #0f172a;\n  }\n}\n";

    #[test]
    fn light_only_override_is_not_flagged() {
        assert!(
            !override_declares_dark_rules(GENERATED_LIGHT_ONLY),
            "a light-only override matches the framework contract and must pass"
        );
    }

    #[test]
    fn override_with_dual_dark_blocks_is_flagged() {
        assert!(
            override_declares_dark_rules(DECLARES_DARK),
            "an override carrying dark rules breaks the light-only contract"
        );
    }

    #[test]
    fn explicit_dark_block_alone_is_flagged() {
        let css = ":root{--rio-bg:#fff}\n:root[data-theme=\"dark\"]{--rio-bg:#0f172a}";
        assert!(override_declares_dark_rules(css));
    }

    #[test]
    fn prefers_color_scheme_block_alone_is_flagged() {
        // This is the one that still *applies* in the browser: the media
        // query matches on the OS setting even though the framework no
        // longer ships a dark palette of its own.
        let css =
    ...
    #[test]
    fn override_with_no_color_tokens_is_ignored() {
        // A spacing/radius-only override cannot repaint a surface.
        let css = ":root{--rio-radius-md:10px}\n@media (prefers-color-scheme: dark){:root{--rio-radius-md:12px}}";
        assert!(!override_declares_dark_rules(css));
    }
