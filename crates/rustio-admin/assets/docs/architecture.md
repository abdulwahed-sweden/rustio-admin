...
- **Shell partials** — `_base.html`, `_topbar.html`, `_sidebar.html`, `_theme.html` (the `AdminTheme` override `<style>`), `_row_actions.html`, `includes/_form_field.html` (single widget render path).
...
```
tokens/ → base/ → components/ → layout/ → pages/ → print/
```
...
**Lock-step invariant:** the `@import` list in `admin/admin.css` and the `ADMIN_CSS` `concat!(include_str!(…), …)` block in `src/admin/routes.rs` must stay in the same order — `print/print.css` is intentionally loaded **last** so its rules override the screen layout. There is no separate responsive fragment; `@media` blocks live inside `layout/console.css` and the `pages/` fragments that need them.
