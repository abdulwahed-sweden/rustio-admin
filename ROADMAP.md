...
- ✅ **Built-in docs pages** — `/admin/docs` lists the framework's three top-level markdown files (`architecture.md`, `modeladmin.md`, `public-api.md`) baked in via `include_str!`. `/admin/docs/<slug>` renders one with `pulldown-cmark` (GFM tables, code blocks, strikethrough, tasklists, smart punctuation). Staff-gated; footer link from every authenticated page. Larger `docs/design/*.md` doctrine files stay out — those are PR-review surface, not in-app reading.
...
A permissions, approval, and audit layer over external AI coding assistants (Claude Code, Copilot, Cursor, …) working inside a rustio-admin project. The position is clarity, not magic: the developer always knows what the AI can do, what it cannot, what needs approval, and what it changed. The AI is an *external actor* governed by the framework's existing permission / audit / builder primitives — **not** an embedded planner (see the ⛔ note below). The full surface ships today and is offline by default; DB-backed identity + audit mirroring is opt-in. Full contract: [`docs/design/DESIGN_AI_ASSISTANT.md`](./docs/design/DESIGN_AI_ASSISTANT.md).

- ✅ **Policy file (`.rustio/ai.toml`).** Version-controlled single source of truth that sorts every capability into Allowed / Needs approval / Blocked. `edit_ai_policy` is itself Blocked, so the AI can't widen its own access; changing a bucket is a reviewed PR edit. `rustio ai init` writes a commented default; an absent file falls back to coded defaults, an unknown key is warned-and-ignored, a malformed file is a hard error.
- ✅ **Three capability buckets + fixed catalogue.** *Allowed* (create models / forms / admin pages, suggest fields, draft migration files), *Needs approval* (apply migration, modify existing table, edit hand-written code, add dependency), *Blocked* (security settings, production deploy, delete data, edit audit log, edit the policy). The catalogue is fixed — the file only *moves* capabilities between buckets. The files-vs-database split is the core safety rule: writing a model + draft migration is Allowed; *applying* it against Postgres needs approval.
- ✅ **Proposal + approval lifecycle.** Suggested → Approved → Applied (or → Rejected). Bucket and required-approvals (0 Allowed / 1 Needs-approval / 2 when the policy lists the capability under `second_approver_for`) are snapshotted at creation so a later policy edit can't change a pending proposal's rules; *distinct* approvers are enforced; `apply` is a separate, explicit step that writes the proposal's staged files (path-traversal-guarded). A Blocked capability is refused at `propose` (and the attempt logged). Proposals are JSON under `.rustio/ai/proposals/`.
- ✅ **Action record + opt-in audit mirroring.** Every suggestion, approval, rejection, apply, and blocked attempt appends to a local `.rustio/ai/log.jsonl` (offline default; `rustio ai log` renders it). Passing `--as <email>` to `approve` / `reject` / `apply` resolves the email to an active `rustio_users` row whose role meets the policy's `approver_role`, records that real user, and **mirrors the decision into `rustio_admin_actions`** (three typed `AuditEvent` variants — `ai_proposal_approved` / `_rejected` / `_applied` — with a UUID-v7 correlation id and `metadata.proposal_id` linking back to the local record). "Who changed it, and who approved it?" has an authenticated answer. `suggested` / `blocked` stay local-only (no authenticated user; `rustio_admin_actions.user_id` is `NOT NULL`).
- ✅ **`rustio ai` command surface.** `status` / `init`, `propose`, `list`, `review`, `approve`, `reject`, `apply`, `log`, and `allow` / `deny` (edits `.rustio/ai.toml` in place, preserving comments, and prints the diff). `status` and the offline lifecycle make no AI call and open no database.
...
- 🟡 **Export / import** — CSV export ships at `GET /admin/<model>/export.csv` (10k-row cap, RFC 4180); CSV import ships at `POST /admin/<model>/import.csv` (multipart upload, 8 MB / 10k-row caps, header columns matched against `AdminField.name`, per-row `AdminOps::create` so framework validation runs unchanged). Result page surfaces inserted / failed counts plus per-row errors. JSON list / detail content negotiation already ships; JSON write-paths return success envelopes. Per-row import dry-run + streaming export for very large tables still planned.
- ✅ **Feature flags** — `rustio_feature_flags` table + `rustio_admin::feature_enabled(db, "key")` helper with a 60-second per-key in-process cache. Administrators manage flags at `/admin/feature_flags` (list with per-row toggle buttons + create form). Linked from the dashboard "Framework tools" aside. `invalidate_cache()` runs after every write so a toggle is observable on the next read without waiting for the TTL.
...
- ✅ **Postgres full-text search opt-in** — `ModelAdmin::search_index_column()` returns `Some("search_vector")` to switch the list-page search box from `ILIKE` OR-loop to `<col> @@ websearch_to_tsquery('english', $N)`. Same search input, indexed lookup; projects supply / maintain the tsvector themselves (generated column or trigger). Default stays `None` (ILIKE path) so existing models are unchanged. Same FTS path runs on the global `⌘K` search palette + the CSV-export filter parser, so every search surface honours the opt-in uniformly.
