...
- 🟡 **Auto-generated OpenAPI page** — `GET /admin/apis/openapi.json` now serves an OpenAPI 3.0 document with per-model component schemas (Bool/Int/String/DateTime/FilePath + nullable variants) and full path coverage for list / create / detail / update / delete. The human-readable HTML index at `/admin/apis` is still planned.
