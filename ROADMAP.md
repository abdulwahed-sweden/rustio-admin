...
- 🟡 **Notifications** — `rustio_notifications` table + public `rustio_admin::send_notification(db, user_id, message, url)` helper. Topbar bell links to `/admin/notifications`, the per-operator list page with a "Mark all read" action. Real-time topbar count badge is still pending (refactor of `BaseContext::new` across 58 call sites is too invasive for this cut); the count surfaces on the list page itself.
