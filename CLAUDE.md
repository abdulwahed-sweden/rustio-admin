...
- The framework ships **two themes** under the RustIO Design System: **light is the default**, with a **dim-slate dark** variant. Theme tokens live in `tokens/colors.css` and `tokens/shadows.css` (elevation), each with a `:root` light block, a `@media (prefers-color-scheme: dark)` block (auto-follow the OS), and explicit `[data-theme="light"]` / `[data-theme="dark"]` blocks that win by source order. An explicit `[data-theme]` choice always overrides the OS preference. (Migration in progress — `DESIGN_DS_MIGRATION.md` tracks the phased rollout; until Phase 1 lands, a compatibility alias bridge in the token files maps pre-DS names like `--rio-accent` onto the new `--rio-rust*` tokens.) The full token philosophy is `docs/design/DESIGN_DOCTRINE.md` (visual identity) and `docs/design/DESIGN_SYSTEM.md` (token ownership). Read both before changing CSS.
