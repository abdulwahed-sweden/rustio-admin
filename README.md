...
<p align="center">
  <strong>Generate real admin systems in Rust.</strong>
</p>

<p align="center">
  Define your domain. RustIO builds the admin foundation. You own the business logic.
</p>
...
<p align="center">
  <img src="docs/assets/admin-shop.png" alt="The Shop admin — a Products list generated by RustIO: products with prices and stock, plus search, filter, sort, and export" width="860">
</p>

<p align="center"><sub><strong>This admin was generated by RustIO Admin.</strong> A real shop admin — products, orders, customers, and payments — rendered from Rust structs over Postgres, with search, filter, sort, and export. Not a mockup, not a frontend template: a generated Rust admin system. <em>(The <a href="./examples/shop"><code>shop</code></a> example.)</em></sub></p>

<p align="center">
  <a href="https://github.com/abdulwahed-sweden/rustio-admin/releases/download/v0.31.0/rustio-admin-film.mp4"><img src="docs/assets/video-poster.png" alt="Watch the RustIO Admin film" width="640"></a>
</p>

<p align="center"><sub>▶ <a href="https://github.com/abdulwahed-sweden/rustio-admin/releases/download/v0.31.0/rustio-admin-film.mp4"><strong>Watch the film</strong></a> — what RustIO generates, in two minutes.</sub></p>

## What is RustIO?

**RustIO Admin generates real admin systems in Rust.**

You define your domain. RustIO builds the admin foundation. You own the business
logic. It is not one dashboard and not a throwaway CRUD screen — from your Rust
models, RustIO generates a complete admin project, and carries the serious parts
real internal systems need from day one:

- **Authentication** — Argon2id passwords, sessions, tokens hashed at rest
- **Permissions & roles** — a real role ladder, deny-by-default
- **Account recovery** — designed as one flow, not patched on afterward
- **Audit** — every authority change writes a typed, correlated record
- **Postgres** — first-class, not an abstraction over five databases
- **Admin UI** — generated from your models, with no frontend build step
- **A real project structure** — code you own, not a low-code prison

No frontend build step. No low-code cage. No AI deciding your business logic.

**Where it comes from.** Django Admin proved developers need a fast way to build
internal tools. Odoo proved companies need systems to run real operations. Python
proved that development speed matters. RustIO is not a clone of any of them — it
takes the idea they proved (*generate the admin foundation quickly*) and rebuilds
it with Rust discipline: generated code you own, explicit control, refusal-first
authority, audit in the core, and no hidden magic. It is for people who looked at
Django Admin and thought *"good idea, but I want Rust,"* at Odoo and thought
*"powerful, but too heavy,"* and at low-code and thought *"fast now, painful
later."*

RustIO Admin does not replace the developer — it gives the developer leverage. It
does not decide your business — it generates the foundation so you build your
...
**Try it. Star it. Fork it. Break it.** Support it if you believe Rust deserves
this layer.

<p align="center"><em>Built in Rust. Governed by engineers. Guarded by design.</em></p>
